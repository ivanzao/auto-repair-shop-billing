    ...
    install(StatusPages) {
        exception<Throwable> { call, cause ->
            val (statusCode, body) = when (cause) {
                is InvalidApprovalTokenException -> BadRequest to cause.toErrorResponseDTO()

                is QuoteNotFoundException -> NotFound to cause.toErrorResponseDTO()
...
private fun extractFieldErrors(cause: BadRequestException): List<FieldError> {
    return when (val rootCause = findRootCause(cause)) {
        is ValueInstantiationException -> {
            val fieldPath = rootCause.path.joinToString(".") { ref -> ref.fieldName ?: "[${ref.index}]" }
